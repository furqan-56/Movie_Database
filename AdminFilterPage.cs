...
namespace Database_Project
{
    public partial class AdminFilterPage : Form
    {
        ...
        private void Delete_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please select a movie to delete.", "No Movie Selected", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            // Get the selected movie's title (or MovieID if you have a unique identifier)
            string selectedMovieTitle = datagrid.SelectedRows[0].Cells["Title"].Value.ToString(); // Assuming "Title" is a column in the DataGridView

            // Ask for confirmation before deletion
            DialogResult dialogResult = MessageBox.Show($"Are you sure you want to delete the movie: {selectedMovieTitle}?",
                                                        "Confirm Deletion",
            ...
            if (dialogResult == DialogResult.Yes)
            {
                try
                {
                    using (MySqlConnection conn = new MySqlConnection(connectionString))
                    {
                        ...
                        // Create the DELETE query (assumes Title is unique, use MovieID if you have one)
                        string query = "DELETE FROM Movies WHERE Title = @Title"; // Change to MovieID if using ID

                        using (MySqlCommand cmd = new MySqlCommand(query, conn))
                        {
                            // Add parameter for Title
                            cmd.Parameters.AddWithValue("@Title", selectedMovieTitle);
